...
class AdtA01Parser {
  ...
  createSegmentParser(context) {
    const typeParser = new TypeParser(context);
    return new SegmentParser(context, typeParser, segmentDescriptors);
  }

  createSegmentIterator(message) {
    const segmentParser = this.createSegmentParser(buildContext(message));
    return new SegmentIterator(message, segmentParser);
  }

  parseMessage(message, model = {}) {
    const iter = this.createSegmentIterator(message);
    ...
    iter.optionalLoop("NK1", (nextOfKin) => {
